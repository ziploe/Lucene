		...
		public void run() {
			try {
				...
				for(;currentShipId<stopShipId;currentShipId++){
					...
					//随机初始位置
					lon=random.nextInt(30)-15;
					lat=random.nextInt(30)-13;
					//随机速度方向
					float mainvx=random.nextFloat();
					float mainvy=random.nextFloat();
					//lon=0;
					//lat=0;
					for(;startSailingTime<totalSailTime;startSailingTime++){
						
						//速度
						vx=(random.nextFloat()-mainvx)/10000.0;
						vy=(random.nextFloat()-mainvy)/10000.0;
						lon=(float)(lon+vx);
						lat=(float)(lat+vy);
