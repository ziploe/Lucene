...
public class TestMultiThreaGeoSearch  {

	private int docPerPage=5;
		...
		try {
			...
			while(command.length()==0 || command.charAt(0)!='q')
			{
				...
				for(;currentDoc<((1+page)*docPerPage>maxDoc?maxDoc:(1+page)*docPerPage);currentDoc++){
					...
					System.out.println(where+"	Id:"+doc.get("id")+"\tlocation:"+doc.get("location")
	...
	public static void main(String[] args){
		TestMultiThreaGeoSearch geoSearch = new TestMultiThreaGeoSearch();
		long lon =Long.parseLong(args[0]);
		long lat =Long.parseLong(args[1]);
		long distance =Long.parseLong(args[2]);
